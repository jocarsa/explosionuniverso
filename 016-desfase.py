...
# Parameters
degree_step = 40  # Degrees between lines
arc_step = 100  # Pixels between left-side arcs (based on horizontal displacement)
...
# New Parameters to Control Centering Feature
center_active_universe = True  # Set to True to center the active universe
x_shift_per_frame = 2  # Pixels to shift past arcs leftwards per frame when centered
...
max_radius = (min(width, height) // 4) * scale_multiplier  # Ensures max_radius is scaled appropriately
...
angles = np.deg2rad(np.arange(1, 360, degree_step))  # Angles for the universe's expansion
...
# Initialize x_offset if centering is active
x_offset = 0  # Initialize to track leftward shift when centered

# Initialize a variable to track when to add the next arc
next_arc_shift = arc_step
...
for frame_idx in range(total_frames):
    ...
    if center_active_universe:
        current_x = width // 2  # Center of the screen
        # Increment x_offset to shift past arcs to the left
        x_offset += x_shift_per_frame
    else:
    ...
    # Determine if a new arc needs to be drawn
    add_new_arc = False
    if not center_active_universe:
        # Original behavior: add a new arc based on horizontal displacement
        if len(arc_snapshots) == 0 or (current_x - arc_snapshots[-1]['arc_x'] >= arc_step):
            add_new_arc = True
    else:
        # Centered behavior: add a new arc based on leftward shift
        if x_offset >= next_arc_shift:
            add_new_arc = True
            next_arc_shift += arc_step
    ...
    # If centering is active, apply x_offset to all past arc_snapshots
    if center_active_universe and x_offset > 0:
        for snapshot in arc_snapshots:
            snapshot['arc_x_shifted'] = snapshot['arc_x'] - x_offset  # Calculate shifted position

    # Remove arcs that have moved beyond the left of the screen when centered
    if center_active_universe:
        arc_snapshots = [snap for snap in arc_snapshots if snap.get('arc_x_shifted', snap['arc_x']) + snap['arc_radius'] > 0]
    else:
        arc_snapshots = [snap for snap in arc_snapshots if snap['arc_x'] + snap['arc_radius'] > 0]

    # Update trail points for all angles
    for angle in trail_points.keys():
        x_offset_trail = int(radius * np.cos(angle))
        y_offset_trail = int(radius * np.sin(angle))
        point = (current_x + x_offset_trail, start_y - y_offset_trail)
    ...
    # Draw the trails for the universe's expansion (gray for past states)
    for angle, points in trail_points.items():
        for i in range(1, len(points)):
            cv2.line(frame, points[i - 1], points[i], rgb_gray, 1, cv2.LINE_AA)

    # Draw arcs and their galaxy snapshots
    for snapshot in arc_snapshots:
        if center_active_universe:
            # Use the shifted x position
            arc_x = snapshot.get('arc_x_shifted', snapshot['arc_x'])
        else:
        ...
        for galaxy_snapshot in snapshot['galaxies']:
            ...
            # Adjust galaxy position if centered
            if center_active_universe:
                galaxy_pos = (galaxy_pos[0] - x_offset, galaxy_pos[1])
            
            # Only draw galaxies within the screen bounds
            if 0 <= galaxy_pos[0] < width and 0 <= galaxy_pos[1] < height:
