...
for frame_idx in range(total_frames):
    ...
    # Draw arcs at positions stored in `arc_positions` (gray for past states)
    for arc_x, arc_radius in arc_positions:
        if arc_x != current_x:  # Avoid recoloring the current circle
            cv2.circle(
                frame,
                (arc_x, start_y),
                arc_radius,  # Use stored radius
                rgb_gray,
                1,
                cv2.LINE_AA,
            )
    ...
    # Note: The universe trails are already handled above with `trail_points` and `arc_positions`.
