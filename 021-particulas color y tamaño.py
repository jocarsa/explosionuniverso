...
particle_trail_color = (255, 255, 255)  # White color for particle trails
...
particles_per_arm = 500  # Number of particles per arm
...
for _ in range(num_galaxies):
    ...
    # ------------------- Assign a Unique Random Color to Each Galaxy's Particles -------------------
    # Generate a random color for the galaxy's particles
    particle_color = (
    ...
    for arm in range(num_arms):
        for p in range(particles_per_arm):
            ...
            position_variation_angle = random.uniform(-0.1, 0.1)  # Small angular variation
            position_variation_radius = random.uniform(-0.02, 0.02)  # Small radial variation
    ...
    galaxies.append({
        ...
        'particle_color': particle_color  # Add particle color
    })
...
for frame_idx in range(total_frames):
    ...
    if len(arc_snapshots) == 0 or current_x - arc_snapshots[-1]['arc_x'] >= arc_step:
        ...
        for galaxy in galaxies:
            ...
            for particle in galaxy['particles']:
                ...
                particle_trail.append((particle_x, particle_y))
    ...
    for snapshot in arc_snapshots:
        ...
        for idx, particle_trail in enumerate(snapshot['particle_trails']):
            galaxy_particle_color = galaxies[idx]['particle_color']  # Retrieve the galaxy's particle color
            for particle_pos in particle_trail:
                cv2.circle(particle_overlay, particle_pos, 1, galaxy_particle_color, -1, cv2.LINE_AA)  # Particle size is set to 1 for trails
    ...
    for galaxy in galaxies:
        ...
        for particle in galaxy['particles']:
            ...
            cv2.circle(frame, (particle_x, particle_y), scaled_particle_size, galaxy['particle_color'], -1, cv2.LINE_AA)
        # ------------------------------------------------------
