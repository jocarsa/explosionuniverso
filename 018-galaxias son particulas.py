...
rgb_gray = (150, 150, 150)  # RGB values for gray color (used for general trails)
...
particles_per_arm = 100  # Number of particles per arm
particle_color = (200, 200, 255)  # Light blue color for particles
particle_size = 1  # Size of each particle
...
for frame_idx in range(total_frames):
    ...
    if len(arc_snapshots) == 0 or current_x - arc_snapshots[-1]['arc_x'] >= arc_step:
        ...
        for galaxy in galaxies:
            ...
            for particle in galaxy['particles']:
                # Scale particle distance based on dynamic particle_system_radius
                particle_distance = particle['distance_norm'] * particle_system_radius
                
                # Compute particle position relative to the galaxy
                particle_x = galaxy_position[0] + int(particle['position_norm'][0] * particle_distance)
                particle_y = galaxy_position[1] - int(particle['position_norm'][1] * particle_distance)  # Inverted Y-axis
    ...
    for snapshot in arc_snapshots:
        ...
        # Draw particle trails for this arc snapshot
        for particle_trail in snapshot['particle_trails']:
            for particle_pos in particle_trail:
                cv2.circle(frame, particle_pos, particle_size, rgb_gray, -1, cv2.LINE_AA)
    ...
    for galaxy in galaxies:
        ...
        for particle in galaxy['particles']:
            ...
            # Draw the particle
            cv2.circle(frame, (particle_x, particle_y), particle_size, particle_color, -1, cv2.LINE_AA)
        # ------------------------------------------------------
